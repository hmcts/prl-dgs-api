...
public class DocumentGeneratorController {
    ...
    public CreatedDocumentInfo createPdf(
        @RequestHeader(value = "Authorization", required = false)
        String authorizationToken,
        @Parameter(name = "GenerateDocumentRequest",description = "JSON object containing "
            + "the templateName and the placeholder text map", required = true)
        ...
        log.info("Document generation requested with templateName [{}], placeholders map of size[{}]",
                 templateData.getTemplate(), templateData.getValues().size());
        return documentManagementService.createDocument(templateData.getTemplate(), templateData.getValues(),
                                                        authorizationToken);
    }

    @Operation(description = "Upload PDF document and saves it in the evidence management. ",
    ...
    public GeneratedDocumentInfo uploadPdf(
        @RequestHeader(value = "Authorization", required = false)
        String authorizationToken,
        @Parameter(name = "UploadDocumentRequest",description = "JSON object containing "
            + "the fileName and the byte document array", required = true)
        @RequestBody
        @Valid
        UploadDocumentRequest uploadDocumentRequest) {
        //This service is internal to PRL system. No need to do service authentication here
        log.info("Upload Document requested with fileName [{}]",
                 uploadDocumentRequest.getFileName());
        return documentManagementService.storeDocument(uploadDocumentRequest.getDocument(), authorizationToken,
                                                       uploadDocumentRequest.getFileName());
    }
